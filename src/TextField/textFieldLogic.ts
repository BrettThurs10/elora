...
export const determineTextFieldStyle = (style?: string) => {
    switch (style) {
        case 'simple':
...
export const determineInputClass = (styleInput?: string) => {
    let baseClass =
    ...
    if (styleInput) {
        return `${determineTextFieldStyle(styleInput)} ${baseClass}`
    } else {
